...
template <typename Dtype>
class SemiContrastLossLayer : public LossLayer<Dtype> {
 public:
  explicit SemiContrastLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  
  virtual inline int ExactNumBottomBlobs() const { return 4; }
  virtual inline const char* type() const { return "SemiContrastLoss"; }
  /**
   * Unlike most loss layers, in the TripletLossLayer we can backpropagate
   * to the first three inputs.
   */
 ...
 protected:
  ...
  // Similarity matrix S
  Blob<Dtype> simi_s;
  Blob<Dtype> dist_sq_;  // square distance
  Blob<Dtype> fea_diff_;
  Blob<Dtype> data_diff_;
  vector<int> vec_sup_anchor;
  vector<int> vec_sup_pos;
  vector<int> vec_sup_neg;
  vector<int> vec_semi_anchor;
  vector<int> vec_semi_pos;
  vector<int> vec_semi_neg;
};

template <typename Dtype>
class SemiRankLossLayer : public LossLayer<Dtype> {
 public:
  explicit SemiRankLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  
  virtual inline int ExactNumBottomBlobs() const { return 4; }
  virtual inline const char* type() const { return "SemiRankLoss"; }
  /**
   * Unlike most loss layers, in the TripletLossLayer we can backpropagate
   * to the first three inputs.
   */
 ...
 protected:
  ...
  Blob<Dtype> fea_diff_;
  Blob<Dtype> data_diff_;
  vector<int> vec_pair_anchor;
  vector<int> vec_pair_pos;
  vector<int> vec_pair_neg;
  vector<int> vec_rank_anchor;
  vector<int> vec_rank_pos;
  vector<int> vec_rank_neg;
};

template <typename Dtype>
class ContrastTripletLossLayer : public LossLayer<Dtype> {
 public:
  explicit ContrastTripletLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "ContrastTripletLoss"; }
  /**
   * Unlike most loss layers, in the TripletLossLayer we can backpropagate
   * to the first three inputs.
   */
 ...
 protected:
  ...
  int iter_count;
  Blob<Dtype> diff_pos_;
  Blob<Dtype> diff_neg_;
  Blob<Dtype> dist_sq_pos_;
  Blob<Dtype> dist_sq_neg_;
  Blob<Dtype> summer_vec_;  // tmp storage for gpu forward pass
};
...
/**
 * @ Multi-label softmax Layer with loss
 */
template <typename Dtype>
class MultiLabelSoftmaxWithLossLayer : public LossLayer<Dtype> {
 public:
  ...
  virtual inline const char* type() const {return "Multilabel_Softmax_Loss"; }
  virtual inline int ExactNumBottomBlobs() const { return -1; }
  virtual inline int MinBottomBlobs() const { return 2; }
  virtual inline int MaxBottomBlobs() const { return 3; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
 ...
 protected:
  /// @copydoc MultiLabelSoftmaxWithLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  //virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  /// The internal SoftmaxLayer used to map predictions to a distribution.
  // shared_ptr<SigmoidLayer<Dtype> > softmax_layer_; author version
  shared_ptr<Layer<Dtype> > softmax_layer_;
  /// prob stores the output probability predictions from the SoftmaxLayer.
  Blob<Dtype> prob_;
  /// bottom vector holder used in call to the underlying SoftmaxLayer::Forward
